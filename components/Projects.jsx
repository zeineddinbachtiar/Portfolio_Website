  ...
  return (
    <div id="projects" style={{ padding: '0 24px', borderBottom: '1px solid var(--rule)' }}>
      ...
      <div style={{
        ...
        <div style={{ borderLeft: '1px solid var(--ink)', padding: '0 0 0 20px', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '.18em', color: 'var(--muted)', fontWeight: 700, display: 'flex', alignItems: 'center', height: '100%' }}>
          2024 — 2026
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr' }}>
          ...
          <a style={heroStyle} onMouseEnter={handleEnter} onMouseLeave={handleLeave}>
            <div className="pc-img" style={{
              ...
              <img
                src={main.img}
                alt={main.title}
                style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', imageRendering: 'auto', objectPosition: 'center top' }}
              />
            ...
            <div className="pc-cat" style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '.18em', color: 'var(--burg)', fontWeight: 900, marginBottom: '5px', transition: 'color .2s' }}>
              {main.cat}
            </div>
            <div className="pc-title" style={{ fontFamily: 'var(--serif)', fontSize: '22px', fontWeight: 700, lineHeight: 1.15, color: 'var(--ink)', marginBottom: '6px', transition: 'color .2s' }}>
              {main.title}
            </div>
            <p className="pc-desc" style={{ fontSize: '14px', color: 'var(--muted)', lineHeight: 1.75, marginBottom: '10px', fontWeight: 500, textAlign: 'justify', transition: 'color .2s' }}>
              {main.desc}
            </p>
            {main.metric && (
              <div className="pc-metric" style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: '14px', color: 'var(--burg)', borderLeft: '2px solid var(--burg)', paddingLeft: '8px', marginBottom: '10px', lineHeight: 1.5, transition: 'all .2s' }}>
                {main.metric}
              </div>
            )}
        ...
        <div style={{
          padding: '0 0 0 20px', display: 'flex', flexDirection: 'column',
            ...
            return (
              <div key={p.slug}
                ...
                onMouseEnter={e => {
                  ...
                  const numEl = e.currentTarget.querySelector('.sm-num')
                  if (numEl) numEl.style.color = 'var(--burg)'
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.paddingLeft = '0'
                  const numEl = e.currentTarget.querySelector('.sm-num')
                  if (numEl) numEl.style.color = 'rgba(12,10,7,.12)'
                }}
              >
                ...
                <div style={{ flex: '1 1 auto', minWidth: 0 }}>
                  <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '.18em', color: 'var(--burg)', fontWeight: 900, marginBottom: '4px' }}>
                    {p.cat}
                  </div>
                  <div style={{ fontFamily: 'var(--serif)', fontSize: '16px', fontWeight: 700, lineHeight: 1.15, marginBottom: '4px' }}>
                    {p.title}
                  </div>
                  <div style={{ fontSize: '13px', color: 'var(--muted)', lineHeight: 1.6, fontWeight: 500 }}>
                    {p.desc}
                  </div>
                </div>
                <div style={{ flex: '0 0 34%', minWidth: '140px', height: '120px', overflow: 'hidden', background: '#E2D9C2', border: '1px solid var(--rule)', flexShrink: 0 }}>
                  <img src={p.img} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', imageRendering: 'auto', objectPosition: 'center top' }} />
                </div>
              </div>
            )
          })}
